        ...
        stage('Slack Or Email Notification') {
            steps {
                script {
                    ...
                    def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://http://3.12.241.46:9000/api/qualitygates/project_status?projectKey=maven" | jq -r .projectStatus.status').trim()
